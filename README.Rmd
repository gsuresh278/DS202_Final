...
dfNull2020 <- dfSelect2020 %>%
  filter(rowSums(is.na(.)) > 0)

dfNull2020$Country
num_rows <- nrow(dfNull2020)
print(num_rows)

# compare the most polluting to this group of countries:
the31Union <- sum(dfNull2020$coTons, na.rm = TRUE)

# Identify the most polluting country
theMostPolluting <- dfSelect2020[dfSelect2020$coTons == max(dfSelect2020$coTons, na.rm = TRUE), ]
theMostPolluting

# Create vectors for x and y values
x_value <- c(theMostPolluting$Country, 'Undefined Group')
y_value <- c(theMostPolluting$coTons, the31Union)

# Create a bar plot
barplot(y_value, names.arg = x_value, col = c('blue', 'green'), main = 'CO2 Emissions Comparison', xlab = 'Country', ylab = 'CO2 Emissions (Tons)')
...
dfSorted2020 <- dfSelect2020 %>% arrange(coTons)
dfSorted2020
i <- 1
countriesSumsCo2 <- 0
dfCountriesSums <- data.frame(matrix(ncol = ncol(dfSorted2020), nrow = 0))
colnames(dfCountriesSums) <- colnames(dfSorted2020)
theMostPolluting <- dfSelect2020[order(-dfSelect2020$coTons), ][1, ]

while (theMostPolluting$coTons > countriesSumsCo2) {
  countriesSumsCo2 <- countriesSumsCo2 + dfSorted2020$coTons[i]
  
  row <- dfSorted2020[i, ]
  dfCountriesSums <- rbind(dfCountriesSums, row)
  
  i <- i + 1
}

cat(paste(i - 1, " ", countriesSumsCo2, "\n"))
dfCountriesSums
...
df2020Big <- dfSelect2020 %>% arrange(desc(coTons)) %>% head(10)

# Filter the original dataset for the selected countries
allYearCO2Big <- df1 %>% filter(Country %in% df2020Big$Country)

# Plot the data using ggplot2
ggplot(allYearCO2Big, aes(x = Year, y = coTons, color = Country)) +
  geom_line(size = 1) +
...
allYearCO219thCenturyBig <- df19thCentury %>% filter(Country %in% df19thCenturyBig$Country)

# Plot the data using ggplot2
ggplot(allYearCO219thCenturyBig, aes(x = Year, y = coTons, color = Country)) +
  geom_line(size = 1) +
...
allYearCO220thCenturyBig <- df20thCentury %>% filter(Country %in% df20thCenturyBig$Country)

# Plot the data using ggplot2
ggplot(allYearCO220thCenturyBig, aes(x = Year, y = coTons, color = Country)) +
  geom_line(size = 1) +
...
allYearCO221stCenturyBig <- df21stCentury %>% filter(Country %in% df21stCenturyBig$Country)

# Plot the data using ggplot2
ggplot(allYearCO221stCenturyBig, aes(x = Year, y = coTons, color = Country)) +
  geom_line(size = 1) +
...
ggplot(df2020BigByArea, aes(x = reorder(Country, -coByArea), y = coByArea, fill = Country)) +
